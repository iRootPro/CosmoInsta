...
def get_exphansion_image(links):
    lst = links.split('/')
    exphansion = lst[-1].split('.')
    return exphansion[-1]


def fetch_habble_images():
    link = get_links_from_habble()[-1]
    exphansion = get_exphansion_image(link)
    image_id = 1
    filename = str(image_id) + '.' + exphansion
...
def main():
    parser = argparse.ArgumentParser(description='Скрипт, который скачивает изображения из репозитариев hubble по указанной коллекции')
